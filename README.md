...
Until Miser is accepted into the default HACS store, add it as a custom repository:
...
After the repository is accepted into the default HACS store, install it directly from **HACS > Integrations** by searching for **Miser PV System Optimiser**.
...
5. Enter system parameters such as battery capacity, inverter power, charger power, and efficiencies.
...
The Read the Docs user guide includes an ApexCharts example for predicted SOC,
forced charge/discharge power, and optimised cost.
